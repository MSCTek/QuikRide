...
namespace CGH.QuikRide.Repository.Entities.QR
{
    ...
    public partial class VehicleStatusTypeConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<VehicleStatusType>
    {
        public VehicleStatusTypeConfiguration()
            : this("dbo")
        {
        }

        public VehicleStatusTypeConfiguration(string schema)
        {
            ToTable("VehicleStatusType", schema);
            HasKey(x => x.VehicleStatusTypeId);

            Property(x => x.VehicleStatusTypeId).HasColumnName(@"VehicleStatusTypeId").HasColumnType("int").IsRequired().HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.None);
            ...
            // Foreign keys
            HasRequired(a => a.LanguageType).WithMany(b => b.VehicleStatusTypes).HasForeignKey(c => c.LanguageTypeId).WillCascadeOnDelete(false); // FK_VehicleStatusType_LanguageType
            InitializePartial();
        }
