...
namespace CGH.QuikRide.Repository.Entities.QR
{
    ...
    public partial class RideServiceTypeConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<RideServiceType>
    {
        public RideServiceTypeConfiguration()
            : this("dbo")
        {
        }

        public RideServiceTypeConfiguration(string schema)
        {
            ToTable("RideServiceType", schema);
            HasKey(x => x.RideServiceTypeId);

            Property(x => x.RideServiceTypeId).HasColumnName(@"RideServiceTypeId").HasColumnType("int").IsRequired().HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.None);
            ...
            // Foreign keys
            HasRequired(a => a.LanguageType).WithMany(b => b.RideServiceTypes).HasForeignKey(c => c.LanguageTypeId).WillCascadeOnDelete(false); // FK_RideServiceType_LanguageType
            InitializePartial();
        }
