...
{
	public partial class RideServiceTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "RideServiceTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.RideServiceType, dtoQR.RideServiceType> _factory 
				= new GenericFactory<entQR.RideServiceType, dtoQR.RideServiceType>();

		public RideServiceTypesQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "RideServiceTypes/{rideServiceTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int rideServiceTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_RideServiceTypeAsync(rideServiceTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "RideServiceTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_RideServiceType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.RideServiceType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "RideServiceTypes/{rideServiceTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int rideServiceTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_RideServiceTypeAsync(rideServiceTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, rideServiceTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "RideServiceTypes/{rideServiceTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int rideServiceTypeId, [FromBody] JsonPatchDocument<dtoQR.RideServiceType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_RideServiceTypeAsync(rideServiceTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "RideServiceTypes", allowedVersion: 1)]
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.RideServiceType newDBItem, ref IRepositoryActionResult<entQR.RideServiceType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.RideServiceType updatedDBItem, ref IRepositoryActionResult<entQR.RideServiceType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.RideServiceType updatedDBItem, ref IRepositoryActionResult<entQR.RideServiceType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.RideServiceType dbItem, int rideServiceTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.RideServiceType> dbItems, ref List<string> filterList);
	}
}