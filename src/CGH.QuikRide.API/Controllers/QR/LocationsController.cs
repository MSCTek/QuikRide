...
{
	public partial class LocationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "LocationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.Location, dtoQR.Location> _factory 
				= new GenericFactory<entQR.Location, dtoQR.Location>();

		public LocationsQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "Locations/{locationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid locationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_LocationAsync(locationId);
		...
		[HttpGet]
		[VersionedRoute(template: "Locations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Location().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.Location).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Locations/{locationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid locationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_LocationAsync(locationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, locationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Locations/{locationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid locationId, [FromBody] JsonPatchDocument<dtoQR.Location> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_LocationAsync(locationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Locations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.Location dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.LocationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Locations/{locationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid locationId, [FromBody] dtoQR.Location dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.Location newDBItem, ref IRepositoryActionResult<entQR.Location> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.Location updatedDBItem, ref IRepositoryActionResult<entQR.Location> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.Location updatedDBItem, ref IRepositoryActionResult<entQR.Location> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.Location dbItem, System.Guid locationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.Location> dbItems, ref List<string> filterList);
	}
}