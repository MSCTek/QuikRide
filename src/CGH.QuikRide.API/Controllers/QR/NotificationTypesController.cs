...
{
	public partial class NotificationTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "NotificationTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.NotificationType, dtoQR.NotificationType> _factory 
				= new GenericFactory<entQR.NotificationType, dtoQR.NotificationType>();

		public NotificationTypesQRController() : base()
		{
		}

		public NotificationTypesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "NotificationTypes/{notificationTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int notificationTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_NotificationTypeAsync(notificationTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "NotificationTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_NotificationType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.NotificationType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "NotificationTypes/{notificationTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int notificationTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_NotificationTypeAsync(notificationTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, notificationTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "NotificationTypes/{notificationTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int notificationTypeId, [FromBody] JsonPatchDocument<dtoQR.NotificationType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_NotificationTypeAsync(notificationTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "NotificationTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.NotificationType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.NotificationTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "NotificationTypes/{notificationTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int notificationTypeId, [FromBody] dtoQR.NotificationType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.NotificationType newDBItem, ref IRepositoryActionResult<entQR.NotificationType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.NotificationType updatedDBItem, ref IRepositoryActionResult<entQR.NotificationType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.NotificationType updatedDBItem, ref IRepositoryActionResult<entQR.NotificationType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.NotificationType dbItem, int notificationTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.NotificationType> dbItems, ref List<string> filterList);
	}
}