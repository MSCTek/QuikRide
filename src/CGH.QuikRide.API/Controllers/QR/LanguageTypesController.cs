...
{
	public partial class LanguageTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "LanguageTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.LanguageType, dtoQR.LanguageType> _factory 
				= new GenericFactory<entQR.LanguageType, dtoQR.LanguageType>();

		public LanguageTypesQRController() : base()
		{
		}

		public LanguageTypesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.LanguageType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpPatch]
		[VersionedRoute(template: "LanguageTypes/{languageTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int languageTypeId, [FromBody] JsonPatchDocument<dtoQR.LanguageType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.LanguageType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "LanguageTypes/{languageTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int languageTypeId, [FromBody] dtoQR.LanguageType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.LanguageType newDBItem, ref IRepositoryActionResult<entQR.LanguageType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.LanguageType updatedDBItem, ref IRepositoryActionResult<entQR.LanguageType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.LanguageType updatedDBItem, ref IRepositoryActionResult<entQR.LanguageType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.LanguageType dbItem, int languageTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.LanguageType> dbItems, ref List<string> filterList);
	}
}