...
{
	public partial class UsersLocationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "UsersLocationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.UsersLocation, dtoQR.UsersLocation> _factory 
				= new GenericFactory<entQR.UsersLocation, dtoQR.UsersLocation>();

		public UsersLocationsQRController() : base()
		{
		}

		public UsersLocationsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "UsersLocations/{userId}/{locationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int userId, System.Guid locationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_UsersLocationAsync(userId, locationId);
		...
		[HttpGet]
		[VersionedRoute(template: "UsersLocations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_UsersLocation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.UsersLocation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "UsersLocations/{userId}/{locationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int userId, System.Guid locationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_UsersLocationAsync(userId, locationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, userId, locationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "UsersLocations/{userId}/{locationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int userId, System.Guid locationId, [FromBody] JsonPatchDocument<dtoQR.UsersLocation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_UsersLocationAsync(userId, locationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "UsersLocations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.UsersLocation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.UserId}/{newDTOItem.LocationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "UsersLocations/{userId}/{locationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int userId, System.Guid locationId, [FromBody] dtoQR.UsersLocation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.UsersLocation newDBItem, ref IRepositoryActionResult<entQR.UsersLocation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.UsersLocation updatedDBItem, ref IRepositoryActionResult<entQR.UsersLocation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.UsersLocation updatedDBItem, ref IRepositoryActionResult<entQR.UsersLocation> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.UsersLocation dbItem, int userId, System.Guid locationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.UsersLocation> dbItems, ref List<string> filterList);
	}
}