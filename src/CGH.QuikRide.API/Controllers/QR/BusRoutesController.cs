...
{
	public partial class BusRoutesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BusRoutesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.BusRoute, dtoQR.BusRoute> _factory 
				= new GenericFactory<entQR.BusRoute, dtoQR.BusRoute>();

		public BusRoutesQRController() : base()
		{
		}

		public BusRoutesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "BusRoutes/{busRouteId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int busRouteId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BusRouteAsync(busRouteId);
		...
		[HttpGet]
		[VersionedRoute(template: "BusRoutes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BusRoute().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.BusRoute).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BusRoutes/{busRouteId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int busRouteId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BusRouteAsync(busRouteId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, busRouteId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BusRoutes/{busRouteId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int busRouteId, [FromBody] JsonPatchDocument<dtoQR.BusRoute> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BusRouteAsync(busRouteId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "BusRoutes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.BusRoute dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BusRouteId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BusRoutes/{busRouteId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int busRouteId, [FromBody] dtoQR.BusRoute dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.BusRoute newDBItem, ref IRepositoryActionResult<entQR.BusRoute> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.BusRoute updatedDBItem, ref IRepositoryActionResult<entQR.BusRoute> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.BusRoute updatedDBItem, ref IRepositoryActionResult<entQR.BusRoute> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.BusRoute dbItem, int busRouteId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.BusRoute> dbItems, ref List<string> filterList);
	}
}