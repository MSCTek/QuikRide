...
{
	public partial class RidePositionsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "RidePositionsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.RidePosition, dtoQR.RidePosition> _factory 
				= new GenericFactory<entQR.RidePosition, dtoQR.RidePosition>();

		public RidePositionsQRController() : base()
		{
		}

		public RidePositionsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "RidePositions/{ridePositionId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid ridePositionId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_RidePositionAsync(ridePositionId);
		...
		[HttpGet]
		[VersionedRoute(template: "RidePositions", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_RidePosition().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.RidePosition).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "RidePositions/{ridePositionId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid ridePositionId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_RidePositionAsync(ridePositionId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, ridePositionId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "RidePositions/{ridePositionId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid ridePositionId, [FromBody] JsonPatchDocument<dtoQR.RidePosition> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_RidePositionAsync(ridePositionId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.RidePositionId = ridePositionId;
		...
		[HttpPost]
		[VersionedRoute(template: "RidePositions", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.RidePosition dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.RidePositionId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "RidePositions/{ridePositionId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid ridePositionId, [FromBody] dtoQR.RidePosition dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.RidePosition newDBItem, ref IRepositoryActionResult<entQR.RidePosition> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.RidePosition updatedDBItem, ref IRepositoryActionResult<entQR.RidePosition> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.RidePosition updatedDBItem, ref IRepositoryActionResult<entQR.RidePosition> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.RidePosition dbItem, System.Guid ridePositionId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.RidePosition> dbItems, ref List<string> filterList);
	}
}