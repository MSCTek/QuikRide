...
{
	public partial class BarcodesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BarcodesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.Barcode, dtoQR.Barcode> _factory 
				= new GenericFactory<entQR.Barcode, dtoQR.Barcode>();

		public BarcodesQRController() : base()
		{
		}

		public BarcodesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Barcodes/{barcodeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid barcodeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BarcodeAsync(barcodeId);
		...
		[HttpGet]
		[VersionedRoute(template: "Barcodes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Barcode().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.Barcode).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Barcodes/{barcodeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid barcodeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BarcodeAsync(barcodeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, barcodeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Barcodes/{barcodeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid barcodeId, [FromBody] JsonPatchDocument<dtoQR.Barcode> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BarcodeAsync(barcodeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Barcodes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.Barcode dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BarcodeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Barcodes/{barcodeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid barcodeId, [FromBody] dtoQR.Barcode dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.Barcode newDBItem, ref IRepositoryActionResult<entQR.Barcode> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.Barcode updatedDBItem, ref IRepositoryActionResult<entQR.Barcode> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.Barcode updatedDBItem, ref IRepositoryActionResult<entQR.Barcode> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.Barcode dbItem, System.Guid barcodeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.Barcode> dbItems, ref List<string> filterList);
	}
}