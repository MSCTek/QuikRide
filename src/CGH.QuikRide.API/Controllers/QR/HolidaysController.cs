...
{
	public partial class HolidaysQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "HolidaysQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.Holiday, dtoQR.Holiday> _factory 
				= new GenericFactory<entQR.Holiday, dtoQR.Holiday>();

		public HolidaysQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "Holidays/{holidayId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int holidayId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_HolidayAsync(holidayId);
		...
		[HttpGet]
		[VersionedRoute(template: "Holidays", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Holiday().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.Holiday).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Holidays/{holidayId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int holidayId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_HolidayAsync(holidayId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, holidayId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Holidays/{holidayId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int holidayId, [FromBody] JsonPatchDocument<dtoQR.Holiday> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_HolidayAsync(holidayId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Holidays", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.Holiday dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.HolidayId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Holidays/{holidayId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int holidayId, [FromBody] dtoQR.Holiday dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.Holiday newDBItem, ref IRepositoryActionResult<entQR.Holiday> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.Holiday updatedDBItem, ref IRepositoryActionResult<entQR.Holiday> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.Holiday updatedDBItem, ref IRepositoryActionResult<entQR.Holiday> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.Holiday dbItem, int holidayId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.Holiday> dbItems, ref List<string> filterList);
	}
}