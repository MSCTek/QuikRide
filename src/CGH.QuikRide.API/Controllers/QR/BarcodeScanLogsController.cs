...
{
	public partial class BarcodeScanLogsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BarcodeScanLogsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.BarcodeScanLog, dtoQR.BarcodeScanLog> _factory 
				= new GenericFactory<entQR.BarcodeScanLog, dtoQR.BarcodeScanLog>();

		public BarcodeScanLogsQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "BarcodeScanLogs/{barcodeScanLogId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid barcodeScanLogId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BarcodeScanLogAsync(barcodeScanLogId);
		...
		[HttpGet]
		[VersionedRoute(template: "BarcodeScanLogs", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BarcodeScanLog().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.BarcodeScanLog).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BarcodeScanLogs/{barcodeScanLogId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid barcodeScanLogId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BarcodeScanLogAsync(barcodeScanLogId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, barcodeScanLogId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BarcodeScanLogs/{barcodeScanLogId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid barcodeScanLogId, [FromBody] JsonPatchDocument<dtoQR.BarcodeScanLog> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BarcodeScanLogAsync(barcodeScanLogId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BarcodeScanLogId = barcodeScanLogId;
		...
		[HttpPost]
		[VersionedRoute(template: "BarcodeScanLogs", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.BarcodeScanLog dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BarcodeScanLogId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BarcodeScanLogs/{barcodeScanLogId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid barcodeScanLogId, [FromBody] dtoQR.BarcodeScanLog dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.BarcodeScanLog newDBItem, ref IRepositoryActionResult<entQR.BarcodeScanLog> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.BarcodeScanLog updatedDBItem, ref IRepositoryActionResult<entQR.BarcodeScanLog> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.BarcodeScanLog updatedDBItem, ref IRepositoryActionResult<entQR.BarcodeScanLog> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.BarcodeScanLog dbItem, System.Guid barcodeScanLogId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.BarcodeScanLog> dbItems, ref List<string> filterList);
	}
}