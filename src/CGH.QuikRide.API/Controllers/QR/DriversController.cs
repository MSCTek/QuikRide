...
{
	public partial class DriversQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "DriversQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.Driver, dtoQR.Driver> _factory 
				= new GenericFactory<entQR.Driver, dtoQR.Driver>();

		public DriversQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "Drivers/{driverId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int driverId)
		{
			try
			{
				...
				var result = await Repo.Delete_DriverAsync(driverId);
		...
		[HttpGet]
		[VersionedRoute(template: "Drivers", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Driver().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.Driver).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Drivers/{driverId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int driverId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_DriverAsync(driverId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, driverId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Drivers/{driverId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int driverId, [FromBody] JsonPatchDocument<dtoQR.Driver> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_DriverAsync(driverId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Drivers", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.Driver dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.DriverId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Drivers/{driverId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int driverId, [FromBody] dtoQR.Driver dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.Driver newDBItem, ref IRepositoryActionResult<entQR.Driver> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.Driver updatedDBItem, ref IRepositoryActionResult<entQR.Driver> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.Driver updatedDBItem, ref IRepositoryActionResult<entQR.Driver> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.Driver dbItem, int driverId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.Driver> dbItems, ref List<string> filterList);
	}
}