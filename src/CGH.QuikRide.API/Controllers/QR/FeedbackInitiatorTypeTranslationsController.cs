...
namespace CGH.QuikRide.API.Controllers.QR
{
	...
	{
			private const string GET_LIST_ROUTE_NAME = "FeedbackInitiatorTypeTranslationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.FeedbackInitiatorTypeTranslation, dtoQR.FeedbackInitiatorTypeTranslation> _factory 
				= new GenericFactory<entQR.FeedbackInitiatorTypeTranslation, dtoQR.FeedbackInitiatorTypeTranslation>();

		public FeedbackInitiatorTypeTranslationsQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "FeedbackInitiatorTypeTranslations/{feedbackInitiatorTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int feedbackInitiatorTypeTranslationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_FeedbackInitiatorTypeTranslationAsync(feedbackInitiatorTypeTranslationId);
		...
		[HttpGet]
		[VersionedRoute(template: "FeedbackInitiatorTypeTranslations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_FeedbackInitiatorTypeTranslation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.FeedbackInitiatorTypeTranslation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "FeedbackInitiatorTypeTranslations/{feedbackInitiatorTypeTranslationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int feedbackInitiatorTypeTranslationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_FeedbackInitiatorTypeTranslationAsync(feedbackInitiatorTypeTranslationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, feedbackInitiatorTypeTranslationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "FeedbackInitiatorTypeTranslations/{feedbackInitiatorTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int feedbackInitiatorTypeTranslationId, [FromBody] JsonPatchDocument<dtoQR.FeedbackInitiatorTypeTranslation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_FeedbackInitiatorTypeTranslationAsync(feedbackInitiatorTypeTranslationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.FeedbackInitiatorTypeTranslationId = feedbackInitiatorTypeTranslationId;
		...
		[HttpPost]
		[VersionedRoute(template: "FeedbackInitiatorTypeTranslations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.FeedbackInitiatorTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.FeedbackInitiatorTypeTranslationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "FeedbackInitiatorTypeTranslations/{feedbackInitiatorTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int feedbackInitiatorTypeTranslationId, [FromBody] dtoQR.FeedbackInitiatorTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.FeedbackInitiatorTypeTranslation newDBItem, ref IRepositoryActionResult<entQR.FeedbackInitiatorTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.FeedbackInitiatorTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.FeedbackInitiatorTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.FeedbackInitiatorTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.FeedbackInitiatorTypeTranslation> result);
