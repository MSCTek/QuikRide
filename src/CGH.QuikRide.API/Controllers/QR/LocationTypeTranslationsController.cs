...
{
	public partial class LocationTypeTranslationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "LocationTypeTranslationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.LocationTypeTranslation, dtoQR.LocationTypeTranslation> _factory 
				= new GenericFactory<entQR.LocationTypeTranslation, dtoQR.LocationTypeTranslation>();

		public LocationTypeTranslationsQRController() : base()
		{
		}

		public LocationTypeTranslationsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "LocationTypeTranslations/{locationTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int locationTypeTranslationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_LocationTypeTranslationAsync(locationTypeTranslationId);
		...
		[HttpGet]
		[VersionedRoute(template: "LocationTypeTranslations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_LocationTypeTranslation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.LocationTypeTranslation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "LocationTypeTranslations/{locationTypeTranslationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int locationTypeTranslationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_LocationTypeTranslationAsync(locationTypeTranslationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, locationTypeTranslationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "LocationTypeTranslations/{locationTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int locationTypeTranslationId, [FromBody] JsonPatchDocument<dtoQR.LocationTypeTranslation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_LocationTypeTranslationAsync(locationTypeTranslationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.LocationTypeTranslationId = locationTypeTranslationId;
		...
		[HttpPost]
		[VersionedRoute(template: "LocationTypeTranslations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.LocationTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.LocationTypeTranslationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "LocationTypeTranslations/{locationTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int locationTypeTranslationId, [FromBody] dtoQR.LocationTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.LocationTypeTranslation newDBItem, ref IRepositoryActionResult<entQR.LocationTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.LocationTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.LocationTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.LocationTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.LocationTypeTranslation> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.LocationTypeTranslation dbItem, int locationTypeTranslationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.LocationTypeTranslation> dbItems, ref List<string> filterList);
	}
}