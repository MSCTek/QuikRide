...
{
	public partial class VehicleTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "VehicleTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.VehicleType, dtoQR.VehicleType> _factory 
				= new GenericFactory<entQR.VehicleType, dtoQR.VehicleType>();

		public VehicleTypesQRController() : base()
		{
		}

		public VehicleTypesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "VehicleTypes/{vehicleTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int vehicleTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_VehicleTypeAsync(vehicleTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "VehicleTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_VehicleType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.VehicleType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "VehicleTypes/{vehicleTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int vehicleTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_VehicleTypeAsync(vehicleTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, vehicleTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "VehicleTypes/{vehicleTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int vehicleTypeId, [FromBody] JsonPatchDocument<dtoQR.VehicleType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_VehicleTypeAsync(vehicleTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "VehicleTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.VehicleType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.VehicleTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "VehicleTypes/{vehicleTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int vehicleTypeId, [FromBody] dtoQR.VehicleType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.VehicleType newDBItem, ref IRepositoryActionResult<entQR.VehicleType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.VehicleType updatedDBItem, ref IRepositoryActionResult<entQR.VehicleType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.VehicleType updatedDBItem, ref IRepositoryActionResult<entQR.VehicleType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.VehicleType dbItem, int vehicleTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.VehicleType> dbItems, ref List<string> filterList);
	}
}