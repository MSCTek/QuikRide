...
{
	public partial class DriverShiftsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "DriverShiftsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.DriverShift, dtoQR.DriverShift> _factory 
				= new GenericFactory<entQR.DriverShift, dtoQR.DriverShift>();

		public DriverShiftsQRController() : base()
		{
		}

		public DriverShiftsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "DriverShifts/{driverShiftId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid driverShiftId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_DriverShiftAsync(driverShiftId);
		...
		[HttpGet]
		[VersionedRoute(template: "DriverShifts", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_DriverShift().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.DriverShift).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "DriverShifts/{driverShiftId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid driverShiftId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_DriverShiftAsync(driverShiftId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, driverShiftId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "DriverShifts/{driverShiftId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid driverShiftId, [FromBody] JsonPatchDocument<dtoQR.DriverShift> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_DriverShiftAsync(driverShiftId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.DriverShiftId = driverShiftId;
		...
		[HttpPost]
		[VersionedRoute(template: "DriverShifts", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.DriverShift dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.DriverShiftId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "DriverShifts/{driverShiftId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid driverShiftId, [FromBody] dtoQR.DriverShift dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.DriverShift newDBItem, ref IRepositoryActionResult<entQR.DriverShift> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.DriverShift updatedDBItem, ref IRepositoryActionResult<entQR.DriverShift> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.DriverShift updatedDBItem, ref IRepositoryActionResult<entQR.DriverShift> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.DriverShift dbItem, System.Guid driverShiftId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.DriverShift> dbItems, ref List<string> filterList);
	}
}