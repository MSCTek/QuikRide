...
{
	public partial class UsersQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "UsersQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.User, dtoQR.User> _factory 
				= new GenericFactory<entQR.User, dtoQR.User>();

		public UsersQRController() : base()
		{
		}

		public UsersQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.User).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpPatch]
		[VersionedRoute(template: "Users/{userId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int userId, [FromBody] JsonPatchDocument<dtoQR.User> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.User dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "Users/{userId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int userId, [FromBody] dtoQR.User dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.User newDBItem, ref IRepositoryActionResult<entQR.User> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.User updatedDBItem, ref IRepositoryActionResult<entQR.User> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.User updatedDBItem, ref IRepositoryActionResult<entQR.User> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.User dbItem, int userId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.User> dbItems, ref List<string> filterList);
	}
}