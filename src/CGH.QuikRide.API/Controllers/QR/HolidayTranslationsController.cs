...
{
	public partial class HolidayTranslationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "HolidayTranslationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.HolidayTranslation, dtoQR.HolidayTranslation> _factory 
				= new GenericFactory<entQR.HolidayTranslation, dtoQR.HolidayTranslation>();

		public HolidayTranslationsQRController() : base()
		{
		}

		public HolidayTranslationsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "HolidayTranslations/{holidayTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int holidayTranslationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_HolidayTranslationAsync(holidayTranslationId);
		...
		[HttpGet]
		[VersionedRoute(template: "HolidayTranslations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_HolidayTranslation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.HolidayTranslation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "HolidayTranslations/{holidayTranslationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int holidayTranslationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_HolidayTranslationAsync(holidayTranslationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, holidayTranslationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "HolidayTranslations/{holidayTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int holidayTranslationId, [FromBody] JsonPatchDocument<dtoQR.HolidayTranslation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_HolidayTranslationAsync(holidayTranslationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.HolidayTranslationId = holidayTranslationId;
		...
		[HttpPost]
		[VersionedRoute(template: "HolidayTranslations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.HolidayTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.HolidayTranslationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "HolidayTranslations/{holidayTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int holidayTranslationId, [FromBody] dtoQR.HolidayTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.HolidayTranslation newDBItem, ref IRepositoryActionResult<entQR.HolidayTranslation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.HolidayTranslation updatedDBItem, ref IRepositoryActionResult<entQR.HolidayTranslation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.HolidayTranslation updatedDBItem, ref IRepositoryActionResult<entQR.HolidayTranslation> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.HolidayTranslation dbItem, int holidayTranslationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.HolidayTranslation> dbItems, ref List<string> filterList);
	}
}