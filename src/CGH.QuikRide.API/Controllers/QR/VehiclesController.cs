...
{
	public partial class VehiclesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "VehiclesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.Vehicle, dtoQR.Vehicle> _factory 
				= new GenericFactory<entQR.Vehicle, dtoQR.Vehicle>();

		public VehiclesQRController() : base()
		{
		}

		public VehiclesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Vehicles/{vehicleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int vehicleId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_VehicleAsync(vehicleId);
		...
		[HttpGet]
		[VersionedRoute(template: "Vehicles", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Vehicle().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.Vehicle).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Vehicles/{vehicleId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int vehicleId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_VehicleAsync(vehicleId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, vehicleId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Vehicles/{vehicleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int vehicleId, [FromBody] JsonPatchDocument<dtoQR.Vehicle> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_VehicleAsync(vehicleId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Vehicles", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.Vehicle dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.VehicleId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Vehicles/{vehicleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int vehicleId, [FromBody] dtoQR.Vehicle dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.Vehicle newDBItem, ref IRepositoryActionResult<entQR.Vehicle> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.Vehicle updatedDBItem, ref IRepositoryActionResult<entQR.Vehicle> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.Vehicle updatedDBItem, ref IRepositoryActionResult<entQR.Vehicle> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.Vehicle dbItem, int vehicleId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.Vehicle> dbItems, ref List<string> filterList);
	}
}