...
{
	public partial class BusRouteTranslationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BusRouteTranslationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.BusRouteTranslation, dtoQR.BusRouteTranslation> _factory 
				= new GenericFactory<entQR.BusRouteTranslation, dtoQR.BusRouteTranslation>();

		public BusRouteTranslationsQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "BusRouteTranslations/{busRouteTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int busRouteTranslationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BusRouteTranslationAsync(busRouteTranslationId);
		...
		[HttpGet]
		[VersionedRoute(template: "BusRouteTranslations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BusRouteTranslation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.BusRouteTranslation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BusRouteTranslations/{busRouteTranslationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int busRouteTranslationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BusRouteTranslationAsync(busRouteTranslationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, busRouteTranslationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BusRouteTranslations/{busRouteTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int busRouteTranslationId, [FromBody] JsonPatchDocument<dtoQR.BusRouteTranslation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BusRouteTranslationAsync(busRouteTranslationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BusRouteTranslationId = busRouteTranslationId;
		...
		[HttpPost]
		[VersionedRoute(template: "BusRouteTranslations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.BusRouteTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BusRouteTranslationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BusRouteTranslations/{busRouteTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int busRouteTranslationId, [FromBody] dtoQR.BusRouteTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.BusRouteTranslation newDBItem, ref IRepositoryActionResult<entQR.BusRouteTranslation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.BusRouteTranslation updatedDBItem, ref IRepositoryActionResult<entQR.BusRouteTranslation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.BusRouteTranslation updatedDBItem, ref IRepositoryActionResult<entQR.BusRouteTranslation> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.BusRouteTranslation dbItem, int busRouteTranslationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.BusRouteTranslation> dbItems, ref List<string> filterList);
	}
}