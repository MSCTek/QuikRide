...
{
	public partial class FeedbackInitiatorTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "FeedbackInitiatorTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.FeedbackInitiatorType, dtoQR.FeedbackInitiatorType> _factory 
				= new GenericFactory<entQR.FeedbackInitiatorType, dtoQR.FeedbackInitiatorType>();

		public FeedbackInitiatorTypesQRController() : base()
		{
		}

		public FeedbackInitiatorTypesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "FeedbackInitiatorTypes/{feedbackInitiatorTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int feedbackInitiatorTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_FeedbackInitiatorTypeAsync(feedbackInitiatorTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "FeedbackInitiatorTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_FeedbackInitiatorType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.FeedbackInitiatorType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "FeedbackInitiatorTypes/{feedbackInitiatorTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int feedbackInitiatorTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_FeedbackInitiatorTypeAsync(feedbackInitiatorTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, feedbackInitiatorTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "FeedbackInitiatorTypes/{feedbackInitiatorTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int feedbackInitiatorTypeId, [FromBody] JsonPatchDocument<dtoQR.FeedbackInitiatorType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_FeedbackInitiatorTypeAsync(feedbackInitiatorTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "FeedbackInitiatorTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.FeedbackInitiatorType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.FeedbackInitiatorTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "FeedbackInitiatorTypes/{feedbackInitiatorTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int feedbackInitiatorTypeId, [FromBody] dtoQR.FeedbackInitiatorType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.FeedbackInitiatorType newDBItem, ref IRepositoryActionResult<entQR.FeedbackInitiatorType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.FeedbackInitiatorType updatedDBItem, ref IRepositoryActionResult<entQR.FeedbackInitiatorType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.FeedbackInitiatorType updatedDBItem, ref IRepositoryActionResult<entQR.FeedbackInitiatorType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.FeedbackInitiatorType dbItem, int feedbackInitiatorTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.FeedbackInitiatorType> dbItems, ref List<string> filterList);
	}
}