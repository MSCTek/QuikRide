...
{
	public partial class BarcodeTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BarcodeTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.BarcodeType, dtoQR.BarcodeType> _factory 
				= new GenericFactory<entQR.BarcodeType, dtoQR.BarcodeType>();

		public BarcodeTypesQRController() : base()
		{
		}

		public BarcodeTypesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "BarcodeTypes/{barcodeTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int barcodeTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BarcodeTypeAsync(barcodeTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "BarcodeTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BarcodeType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.BarcodeType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BarcodeTypes/{barcodeTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int barcodeTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BarcodeTypeAsync(barcodeTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, barcodeTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BarcodeTypes/{barcodeTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int barcodeTypeId, [FromBody] JsonPatchDocument<dtoQR.BarcodeType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BarcodeTypeAsync(barcodeTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BarcodeTypeId = barcodeTypeId;
		...
		[HttpPost]
		[VersionedRoute(template: "BarcodeTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.BarcodeType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BarcodeTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BarcodeTypes/{barcodeTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int barcodeTypeId, [FromBody] dtoQR.BarcodeType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.BarcodeType newDBItem, ref IRepositoryActionResult<entQR.BarcodeType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.BarcodeType updatedDBItem, ref IRepositoryActionResult<entQR.BarcodeType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.BarcodeType updatedDBItem, ref IRepositoryActionResult<entQR.BarcodeType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.BarcodeType dbItem, int barcodeTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.BarcodeType> dbItems, ref List<string> filterList);
	}
}