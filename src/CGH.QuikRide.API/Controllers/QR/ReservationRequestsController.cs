...
{
	public partial class ReservationRequestsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "ReservationRequestsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.ReservationRequest, dtoQR.ReservationRequest> _factory 
				= new GenericFactory<entQR.ReservationRequest, dtoQR.ReservationRequest>();

		public ReservationRequestsQRController() : base()
		{
		}

		public ReservationRequestsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "ReservationRequests/{reservationRequestId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid reservationRequestId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_ReservationRequestAsync(reservationRequestId);
		...
		[HttpGet]
		[VersionedRoute(template: "ReservationRequests", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_ReservationRequest().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.ReservationRequest).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "ReservationRequests/{reservationRequestId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid reservationRequestId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_ReservationRequestAsync(reservationRequestId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, reservationRequestId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "ReservationRequests/{reservationRequestId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid reservationRequestId, [FromBody] JsonPatchDocument<dtoQR.ReservationRequest> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_ReservationRequestAsync(reservationRequestId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.ReservationRequestId = reservationRequestId;
		...
		[HttpPost]
		[VersionedRoute(template: "ReservationRequests", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.ReservationRequest dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.ReservationRequestId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "ReservationRequests/{reservationRequestId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid reservationRequestId, [FromBody] dtoQR.ReservationRequest dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.ReservationRequest newDBItem, ref IRepositoryActionResult<entQR.ReservationRequest> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.ReservationRequest updatedDBItem, ref IRepositoryActionResult<entQR.ReservationRequest> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.ReservationRequest updatedDBItem, ref IRepositoryActionResult<entQR.ReservationRequest> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.ReservationRequest dbItem, System.Guid reservationRequestId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.ReservationRequest> dbItems, ref List<string> filterList);
	}
}