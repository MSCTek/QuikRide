...
{
	public partial class FeedbackTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "FeedbackTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.FeedbackType, dtoQR.FeedbackType> _factory 
				= new GenericFactory<entQR.FeedbackType, dtoQR.FeedbackType>();

		public FeedbackTypesQRController() : base()
		{
		}

		public FeedbackTypesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "FeedbackTypes/{feedbackTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int feedbackTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_FeedbackTypeAsync(feedbackTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "FeedbackTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_FeedbackType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.FeedbackType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "FeedbackTypes/{feedbackTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int feedbackTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_FeedbackTypeAsync(feedbackTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, feedbackTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "FeedbackTypes/{feedbackTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int feedbackTypeId, [FromBody] JsonPatchDocument<dtoQR.FeedbackType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_FeedbackTypeAsync(feedbackTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.FeedbackTypeId = feedbackTypeId;
		...
		[HttpPost]
		[VersionedRoute(template: "FeedbackTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.FeedbackType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.FeedbackTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "FeedbackTypes/{feedbackTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int feedbackTypeId, [FromBody] dtoQR.FeedbackType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.FeedbackType newDBItem, ref IRepositoryActionResult<entQR.FeedbackType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.FeedbackType updatedDBItem, ref IRepositoryActionResult<entQR.FeedbackType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.FeedbackType updatedDBItem, ref IRepositoryActionResult<entQR.FeedbackType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.FeedbackType dbItem, int feedbackTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.FeedbackType> dbItems, ref List<string> filterList);
	}
}