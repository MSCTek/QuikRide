...
{
	public partial class GenderTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "GenderTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.GenderType, dtoQR.GenderType> _factory 
				= new GenericFactory<entQR.GenderType, dtoQR.GenderType>();

		public GenderTypesQRController() : base()
		{
		}

		public GenderTypesQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.GenderType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpPatch]
		[VersionedRoute(template: "GenderTypes/{genderTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int genderTypeId, [FromBody] JsonPatchDocument<dtoQR.GenderType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.GenderType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "GenderTypes/{genderTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int genderTypeId, [FromBody] dtoQR.GenderType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.GenderType updatedDBItem, ref IRepositoryActionResult<entQR.GenderType> result);
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.GenderType> dbItems, ref List<string> filterList);
	}
}