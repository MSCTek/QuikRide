...
{
	public partial class BusRouteStopsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BusRouteStopsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.BusRouteStop, dtoQR.BusRouteStop> _factory 
				= new GenericFactory<entQR.BusRouteStop, dtoQR.BusRouteStop>();

		public BusRouteStopsQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "BusRouteStops/{busRouteStopId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int busRouteStopId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BusRouteStopAsync(busRouteStopId);
		...
		[HttpGet]
		[VersionedRoute(template: "BusRouteStops", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BusRouteStop().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.BusRouteStop).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BusRouteStops/{busRouteStopId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int busRouteStopId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BusRouteStopAsync(busRouteStopId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, busRouteStopId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BusRouteStops/{busRouteStopId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int busRouteStopId, [FromBody] JsonPatchDocument<dtoQR.BusRouteStop> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BusRouteStopAsync(busRouteStopId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BusRouteStopId = busRouteStopId;
		...
		[HttpPost]
		[VersionedRoute(template: "BusRouteStops", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.BusRouteStop dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BusRouteStopId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BusRouteStops/{busRouteStopId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int busRouteStopId, [FromBody] dtoQR.BusRouteStop dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.BusRouteStop newDBItem, ref IRepositoryActionResult<entQR.BusRouteStop> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.BusRouteStop updatedDBItem, ref IRepositoryActionResult<entQR.BusRouteStop> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.BusRouteStop updatedDBItem, ref IRepositoryActionResult<entQR.BusRouteStop> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.BusRouteStop dbItem, int busRouteStopId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.BusRouteStop> dbItems, ref List<string> filterList);
	}
}