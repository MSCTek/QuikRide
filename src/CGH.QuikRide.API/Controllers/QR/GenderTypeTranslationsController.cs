...
{
	public partial class GenderTypeTranslationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "GenderTypeTranslationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.GenderTypeTranslation, dtoQR.GenderTypeTranslation> _factory 
				= new GenericFactory<entQR.GenderTypeTranslation, dtoQR.GenderTypeTranslation>();

		public GenderTypeTranslationsQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "GenderTypeTranslations/{genderTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int genderTypeTranslationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_GenderTypeTranslationAsync(genderTypeTranslationId);
		...
		[HttpGet]
		[VersionedRoute(template: "GenderTypeTranslations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_GenderTypeTranslation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.GenderTypeTranslation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "GenderTypeTranslations/{genderTypeTranslationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int genderTypeTranslationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_GenderTypeTranslationAsync(genderTypeTranslationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, genderTypeTranslationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "GenderTypeTranslations/{genderTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int genderTypeTranslationId, [FromBody] JsonPatchDocument<dtoQR.GenderTypeTranslation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_GenderTypeTranslationAsync(genderTypeTranslationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.GenderTypeTranslationId = genderTypeTranslationId;
		...
		[HttpPost]
		[VersionedRoute(template: "GenderTypeTranslations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.GenderTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.GenderTypeTranslationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "GenderTypeTranslations/{genderTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int genderTypeTranslationId, [FromBody] dtoQR.GenderTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.GenderTypeTranslation newDBItem, ref IRepositoryActionResult<entQR.GenderTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.GenderTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.GenderTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.GenderTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.GenderTypeTranslation> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.GenderTypeTranslation dbItem, int genderTypeTranslationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.GenderTypeTranslation> dbItems, ref List<string> filterList);
	}
}