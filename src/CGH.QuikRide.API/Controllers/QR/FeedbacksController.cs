...
{
	public partial class FeedbacksQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "FeedbacksQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.Feedback, dtoQR.Feedback> _factory 
				= new GenericFactory<entQR.Feedback, dtoQR.Feedback>();

		public FeedbacksQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "Feedbacks/{feedbackId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid feedbackId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_FeedbackAsync(feedbackId);
		...
		[HttpGet]
		[VersionedRoute(template: "Feedbacks", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Feedback().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.Feedback).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Feedbacks/{feedbackId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid feedbackId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_FeedbackAsync(feedbackId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, feedbackId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Feedbacks/{feedbackId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid feedbackId, [FromBody] JsonPatchDocument<dtoQR.Feedback> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_FeedbackAsync(feedbackId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Feedbacks", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.Feedback dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.FeedbackId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Feedbacks/{feedbackId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid feedbackId, [FromBody] dtoQR.Feedback dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.Feedback newDBItem, ref IRepositoryActionResult<entQR.Feedback> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.Feedback updatedDBItem, ref IRepositoryActionResult<entQR.Feedback> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.Feedback updatedDBItem, ref IRepositoryActionResult<entQR.Feedback> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.Feedback dbItem, System.Guid feedbackId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.Feedback> dbItems, ref List<string> filterList);
	}
}