...
{
	public partial class ReservationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "ReservationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.Reservation, dtoQR.Reservation> _factory 
				= new GenericFactory<entQR.Reservation, dtoQR.Reservation>();

		public ReservationsQRController() : base()
		{
		}

		public ReservationsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Reservations/{reservationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid reservationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_ReservationAsync(reservationId);
		...
		[HttpGet]
		[VersionedRoute(template: "Reservations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Reservation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.Reservation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Reservations/{reservationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid reservationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_ReservationAsync(reservationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, reservationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Reservations/{reservationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid reservationId, [FromBody] JsonPatchDocument<dtoQR.Reservation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_ReservationAsync(reservationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.ReservationId = reservationId;
		...
		[HttpPost]
		[VersionedRoute(template: "Reservations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.Reservation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.ReservationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Reservations/{reservationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid reservationId, [FromBody] dtoQR.Reservation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.Reservation newDBItem, ref IRepositoryActionResult<entQR.Reservation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.Reservation updatedDBItem, ref IRepositoryActionResult<entQR.Reservation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.Reservation updatedDBItem, ref IRepositoryActionResult<entQR.Reservation> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.Reservation dbItem, System.Guid reservationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.Reservation> dbItems, ref List<string> filterList);
	}
}