...
namespace CGH.QuikRide.API.Controllers.QR
{
	...
	{
			private const string GET_LIST_ROUTE_NAME = "UserRewardAccountsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.UserRewardAccount, dtoQR.UserRewardAccount> _factory 
				= new GenericFactory<entQR.UserRewardAccount, dtoQR.UserRewardAccount>();

		public UserRewardAccountsQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "UserRewardAccounts/{userId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int userId)
		{
			try
			{
				...
				var result = await Repo.Delete_UserRewardAccountAsync(userId);
		...
		[HttpGet]
		[VersionedRoute(template: "UserRewardAccounts", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_UserRewardAccount().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.UserRewardAccount).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "UserRewardAccounts/{userId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int userId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_UserRewardAccountAsync(userId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "UserRewardAccounts/{userId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int userId, [FromBody] JsonPatchDocument<dtoQR.UserRewardAccount> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_UserRewardAccountAsync(userId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "UserRewardAccounts", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.UserRewardAccount dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "UserRewardAccounts/{userId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int userId, [FromBody] dtoQR.UserRewardAccount dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.UserRewardAccount newDBItem, ref IRepositoryActionResult<entQR.UserRewardAccount> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.UserRewardAccount updatedDBItem, ref IRepositoryActionResult<entQR.UserRewardAccount> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.UserRewardAccount updatedDBItem, ref IRepositoryActionResult<entQR.UserRewardAccount> result);
