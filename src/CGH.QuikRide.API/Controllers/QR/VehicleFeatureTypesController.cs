...
{
	public partial class VehicleFeatureTypesQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "VehicleFeatureTypesQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.VehicleFeatureType, dtoQR.VehicleFeatureType> _factory 
				= new GenericFactory<entQR.VehicleFeatureType, dtoQR.VehicleFeatureType>();

		public VehicleFeatureTypesQRController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "VehicleFeatureTypes/{vehicleFeatureTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int vehicleFeatureTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_VehicleFeatureTypeAsync(vehicleFeatureTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "VehicleFeatureTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_VehicleFeatureType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.VehicleFeatureType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "VehicleFeatureTypes/{vehicleFeatureTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int vehicleFeatureTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_VehicleFeatureTypeAsync(vehicleFeatureTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, vehicleFeatureTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "VehicleFeatureTypes/{vehicleFeatureTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int vehicleFeatureTypeId, [FromBody] JsonPatchDocument<dtoQR.VehicleFeatureType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_VehicleFeatureTypeAsync(vehicleFeatureTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "VehicleFeatureTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.VehicleFeatureType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.VehicleFeatureTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "VehicleFeatureTypes/{vehicleFeatureTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int vehicleFeatureTypeId, [FromBody] dtoQR.VehicleFeatureType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.VehicleFeatureType newDBItem, ref IRepositoryActionResult<entQR.VehicleFeatureType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.VehicleFeatureType updatedDBItem, ref IRepositoryActionResult<entQR.VehicleFeatureType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.VehicleFeatureType updatedDBItem, ref IRepositoryActionResult<entQR.VehicleFeatureType> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.VehicleFeatureType dbItem, int vehicleFeatureTypeId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.VehicleFeatureType> dbItems, ref List<string> filterList);
	}
}