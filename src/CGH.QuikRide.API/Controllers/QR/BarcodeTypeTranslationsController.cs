...
{
	public partial class BarcodeTypeTranslationsQRController : QRBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BarcodeTypeTranslationsQRList";
			private const int maxPageSize = 100;

			private GenericFactory<entQR.BarcodeTypeTranslation, dtoQR.BarcodeTypeTranslation> _factory 
				= new GenericFactory<entQR.BarcodeTypeTranslation, dtoQR.BarcodeTypeTranslation>();

		public BarcodeTypeTranslationsQRController() : base()
		{
		}

		public BarcodeTypeTranslationsQRController(ILoggingService log, IQRRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "BarcodeTypeTranslations/{barcodeTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int barcodeTypeTranslationId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BarcodeTypeTranslationAsync(barcodeTypeTranslationId);
		...
		[HttpGet]
		[VersionedRoute(template: "BarcodeTypeTranslations", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BarcodeTypeTranslation().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entQR.BarcodeTypeTranslation).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BarcodeTypeTranslations/{barcodeTypeTranslationId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int barcodeTypeTranslationId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BarcodeTypeTranslationAsync(barcodeTypeTranslationId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, barcodeTypeTranslationId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BarcodeTypeTranslations/{barcodeTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int barcodeTypeTranslationId, [FromBody] JsonPatchDocument<dtoQR.BarcodeTypeTranslation> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BarcodeTypeTranslationAsync(barcodeTypeTranslationId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BarcodeTypeTranslationId = barcodeTypeTranslationId;
		...
		[HttpPost]
		[VersionedRoute(template: "BarcodeTypeTranslations", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoQR.BarcodeTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BarcodeTypeTranslationId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BarcodeTypeTranslations/{barcodeTypeTranslationId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int barcodeTypeTranslationId, [FromBody] dtoQR.BarcodeTypeTranslation dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CGH.QuikRide.Repository.Entities.QR.BarcodeTypeTranslation newDBItem, ref IRepositoryActionResult<entQR.BarcodeTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CGH.QuikRide.Repository.Entities.QR.BarcodeTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.BarcodeTypeTranslation> result);

		partial void RunCustomLogicAfterUpdatePut(ref CGH.QuikRide.Repository.Entities.QR.BarcodeTypeTranslation updatedDBItem, ref IRepositoryActionResult<entQR.BarcodeTypeTranslation> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entQR.BarcodeTypeTranslation dbItem, int barcodeTypeTranslationId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entQR.BarcodeTypeTranslation> dbItems, ref List<string> filterList);
	}
}