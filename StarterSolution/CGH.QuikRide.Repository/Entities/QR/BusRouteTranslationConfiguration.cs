...
namespace CGH.QuikRide.Repository.Entities.QR
{
    ...
    public partial class BusRouteTranslationConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<BusRouteTranslation>
    {
        public BusRouteTranslationConfiguration()
            : this("dbo")
        {
        }

        public BusRouteTranslationConfiguration(string schema)
        {
            ToTable("BusRouteTranslation", schema);
            HasKey(x => x.BusRouteTranslationId);

            Property(x => x.BusRouteTranslationId).HasColumnName(@"BusRouteTranslationId").HasColumnType("int").IsRequired().HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.Identity);
            ...
            // Foreign keys
            HasRequired(a => a.BusRoute).WithMany(b => b.BusRouteTranslations).HasForeignKey(c => c.BusRouteId); // FK_BusRouteTranslation_BusRoute
            HasRequired(a => a.LanguageType).WithMany(b => b.BusRouteTranslations).HasForeignKey(c => c.LanguageTypeId).WillCascadeOnDelete(false); // FK_BusRouteTranslation_LanguageType
            InitializePartial();
        }
