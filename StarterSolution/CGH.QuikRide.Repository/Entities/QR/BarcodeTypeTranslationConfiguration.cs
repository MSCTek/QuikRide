...
namespace CGH.QuikRide.Repository.Entities.QR
{
    ...
    public partial class BarcodeTypeTranslationConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<BarcodeTypeTranslation>
    {
        public BarcodeTypeTranslationConfiguration()
            : this("dbo")
        {
        }

        public BarcodeTypeTranslationConfiguration(string schema)
        {
            ToTable("BarcodeTypeTranslation", schema);
            HasKey(x => x.BarcodeTypeTranslationId);

            Property(x => x.BarcodeTypeTranslationId).HasColumnName(@"BarcodeTypeTranslationId").HasColumnType("int").IsRequired().HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.Identity);
            ...
            // Foreign keys
            HasRequired(a => a.BarcodeType).WithMany(b => b.BarcodeTypeTranslations).HasForeignKey(c => c.BarcodeTypeId); // FK_BarcodeTypeTranslation_BarcodeType
            HasRequired(a => a.LanguageType).WithMany(b => b.BarcodeTypeTranslations).HasForeignKey(c => c.LanguageTypeId).WillCascadeOnDelete(false); // FK_BarcodeTypeTranslation_LanguageType
            InitializePartial();
        }
